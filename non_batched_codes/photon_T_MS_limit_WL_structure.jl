# In this code, I have modified the Wilson line structure to effectively include the gluon emission after the shockwave. This is required in the large M_x limit.
# The modification is N_012 -> N_012 - N_01
# The previous code photon_T_large_Nc_large_MX.jl is be incorrect since it used S_012 -> S_012 - S_01 which is not the same as the above modification.
using JSON
using MCIntegration
...
function S012(a, x20, th20, x21, th21) #Large Nc limit
    return S(a,x20)*S(a,x21)
end

function N012(a,x20,th20,x21,th21)
    return 1 - S012(a,x20,th20,x21,th21);
end
...
function integrand(Nc, CF, a, Q, beta, z0, z1, x20, x20b, th20b, x21, th21, x21b, th21b)
    ...
    x10b = sqrt(x20b^2 + x21b^2 - 2.0*x20b*x21b*cos(th20b - th21b));
    # Using modified Wilson line structure to effectively include post-shockwave gluon emission
    # Corresponds to replacing N012 -> N012 - N01 and the same for conjugate
    # See comment by Jani in the mattermost server
    return measure*GNLOT(Q, beta, z0, z1, x20, th20, x20b, th20b, x21, th21, x21b, th21b)*(N012(a, x20, th20, x21, th21) - N(a,x10))*(N012(a, x20b, th20b, x21b, th21b) - N(a,x10b));
end
...
function main()
    ...
    println("Description: Monte Carlo integral for trip T contribution (modified WL structure for MS limit at beta->0)");
